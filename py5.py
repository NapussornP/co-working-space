...
name = StringVar()
date = StringVar()
result_current = StringVar()
month = StringVar()
result_month = StringVar()
error = StringVar()
...
def calprice(): #ใส่พารา
    def price(start,end):
        '''global start,end,hr,total_price'''
        while True:
            ...
            if 7<=st_hr<=24 and 7<=e_hr<=24 and 0<=st_min<=60 and 0<=e_min<=60:
                ...
                error.set('please enter Start time and End time again ')
...
e = Label(mywin,textvariable=error,font=('Tahoma',14),fg='red')
e.grid(row=7,column=1,pady=7)
...
def income_month():
    ...
    for col in mylist:
        if m == ['Feb']: 
            sum_current += eval(col[6],row[:4])

    result_current.set('Current income: {} THB'.format(sum_current))
...
R_cur = Label(mywin, textvariable=result_current,font=('Tahoma',11),fg='red')
R_cur.grid(row=5,column=2,columnspan=2,pady=7) 
