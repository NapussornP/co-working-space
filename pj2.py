...
#--------------------------------------------------------------------owner----------------------------------------------------------
def for_owner(): #.....don't forget put display income......
    myowner=Tk()
    ...
    m = ttk.Combobox(myowner,width=8,textvariable=month)
    m['values'] = ('Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec')
    m.grid(row=1,column=2,pady=6)
    

    #--------------------------------------------------------------------earned income----------------------------------------------------------
    '''def income_day():
        df = pd.read_csv('working.csv',usecols=['date','month'])
        df['price']=df['price'].astype('int')

        DataFrame1 = DataFrame.loc[:,(DataFrame)]'''
    #--------------------------------------------------------------income month------------------------------------------------------------
    def income_month():
    ...
    btmonth = Button(myowner,text='Calculate',width=10,command=income_month) #command
    btmonth.grid(row=3,column=2,padx=4,pady=6) 

    R_day = Label(myowner, textvariable=result_day,font=('Tahoma',11))
    R_day.grid(row=4,column=1,pady=7)    
