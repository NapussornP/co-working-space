...
def for_owner(): #.....don't forget put display income......
    ...
    def earned():
        col_list = ["M/D/Y"]
        df = pandas.read_csv("working.csv", usecols=col_list)
        
        sum_day = 0
        while col_list == date:
            col_list1 = ['price']
            df = pandas.read_csv("working.csv", usecols=col_list1)
            sum_day += eval(col_list1)
    ...
    btday = Button(myowner,text='Calculate',width=10,command=earned)  #ยังไม่ใส่command
